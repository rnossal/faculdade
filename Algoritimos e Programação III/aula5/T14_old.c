...
// (Peguei tua função mesmo, Elgio. Ela é útil)
...
// Compara igualdade entre duas strings (case sensitive).
int equals(char str[], char strToCompare[]) {
	int i;
	if (length(str) != length(strToCompare)) return 0;

	for (i = 0; i < length(str); i++) {
		if (str[i] != strToCompare[i]) return 0;
	}
...
int equalsNoCase(char str[], char strToCompare[]) {
	int i;
	char a, b;
	...
	// char comp2[MAXCHARS];
	//
	// copy(comp1, upper(str), MAXCHARS);
	// copy(comp2, upper(strToCompare), MAXCHARS);
	//
	// return equals(comp1, comp2);
}

// Reverte a string passada e retorna a nova string.
...
char *formatToOnlyChars(char str[]) {
	int i,
		j = 0;
	char buffer[MAXCHARS];
	char *strOut = buffer;
...
main() {
	...
	while (1) { // Loop infinito porque é mais rápido pelo uso da lógica atual (se eu quebrar o loop direito).
		printf("# [%d] Digite frase de até %d caracteres.\n", rel.phrases + 1, MAXCHARS);
		printf("# [%d] Digite \"%s\" para encerrar o programa\n", rel.phrases + 1, exitPhrase);
		read(readPhrase, MAXCHARS);

		if (equalsNoCase(exitPhrase, readPhrase)) break;

		char phraseOnlyChar[MAXCHARS];
		char phraseOnlyCharReverse[MAXCHARS];
		copy(phraseOnlyChar, formatToOnlyChars(readPhrase), MAXCHARS);
		copy(phraseOnlyCharReverse, formatToOnlyChars(reverse(readPhrase)), MAXCHARS);

		countRead = countCase(readPhrase);

		if (equalsNoCase(phraseOnlyChar, phraseOnlyCharReverse)) {
